...
// Optional config: provide contract addresses or deployer EOAs via env vars to skip scanning.

// Configure static Vybe contract addresses directly.
// Supports VYBE_CONTRACT_ADDRESSES / NEXT_PUBLIC_VYBE_CONTRACT_ADDRESSES (JSON/CSV).
export function getConfiguredContractAddresses(): `0x${string}`[] {
    const server = process.env.VYBE_CONTRACT_ADDRESSES;
    const client = process.env.NEXT_PUBLIC_VYBE_CONTRACT_ADDRESSES;

    const raw = server && server.trim().length > 0 ? server : client;
    ...
    for (const p of parts) {
        if (p.startsWith("0x")) addrs.push(p as `0x${string}`);
    }
    return addrs;
}
...
export function getConfiguredDeployerAddresses(): `0x${string}`[] {
    ...
    const single = process.env.NEXT_PUBLIC_DEPLOYER_ADDRESS as
        | `0x${string}`
        | undefined;
    const addrs: `0x${string}`[] = [];
...
export async function discoverVybeContractsFromDeployers(
    ...
    const staticContracts = getConfiguredContractAddresses();
    if (staticContracts.length > 0) {
        const verified: `0x${string}`[] = [];
        for (const addr of staticContracts) {
            try {
                ...
                // Skip invalid/unknown addresses silently.
            }
        }
        if (verified.length > 0) return verified;
        // If none verified, fall through to discovery.
    }
    ...
    if (opts?.startBlock !== undefined) startBlock = opts.startBlock;
        ...
        startBlock =
            latest > BigInt(maxBlocks) ? latest - BigInt(maxBlocks) : BigInt(0);
    ...
    try {
        const logs = await client.getLogs({
            ...
            toBlock: latest,
            event: VYBE_CONTRACT_ABI.find(
                (e: any) => e.type === "event" && e.name === "MarketCreated"
            ) as any,
    ...
    const deployers = new Set(
        getConfiguredDeployerAddresses().map((a) => a.toLowerCase())
    );
